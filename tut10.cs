...
namespace Tutorial
{
  class Program
  {
    ...
    static public SubmatrixInfo SquareSubmatrix(int[,] matrix)
    {
      ...
      // best[i,j] == size <=> the largest square submatrix with all 1s
      // that ends at pos (i,j) has the given size

      SubmatrixInfo largest = new();
      for (int i = 0; i < rows; i++)
      {
        for (int j = 0; j < cols; j++)
        {
          int curBest = 0;
          if (matrix[i, j] == 1) // Matrix could potentially be extended
          {
            if (i == 0 || j == 0)
              curBest = matrix[i, j]; // Largest submatrix at the edges has size 0 or 1
            else
              curBest = 1 + Math.Min(best[i - 1, j], Math.Min(best[i, j - 1], best[i - 1, j - 1]));
          }
          best[i, j] = curBest;
          if (curBest > largest.Size)
          {
            largest = new() { OffsetX = i - curBest + 1, OffsetY = j - curBest + 1, Size = curBest };
          }
        }
      }
