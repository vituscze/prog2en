...
namespace Tutorial
{
  class Program
  {
    ...
    */
    ...
    //
    // What if virtual/override keywords weren't there?
    ...
    // In the lecture we saw a class IntStack with a subclass AvgStack including
    // a property double avg that returns the average of all values on the stack in O(1).

    // Write a subclass MaxStack that is like IntStack but also includes a property
    // double max that returns the maximum of all values on the stack in O(1). If the
    // stack is empty, the max property should return the value double.NaN.
    class IntStack
    {
        int[] a;
        int n;
        ...
        public static bool operator ! (IntStack s) => s.count == 0;
    }

    // How to do max in O(1)?

    // class MaxStack : IntStack
    // { }
